...
Connector::Connector(EventLoop *l, TCPClient *client)
    : status_(kDisconnected), loop_(l), owner_tcp_client_(client),
...
Connector::~Connector() {
  ...
  if (status_ == kDNSResolving) {
    ...
    // But the sockfd of unconnected tcp-connections need to be closed by
    // myself.
...
void Connector::Start() {
  DLOG_TRACE << "Try to connect " << remote_addr_
             << " status=" << StatusToString();
  assert(loop_->IsInLoopThread());

  timer_.reset(new TimerEventWatcher(
      loop_, std::bind(&Connector::OnConnectTimeout, shared_from_this()),
      timeout_));
  timer_->Init();
  timer_->AsyncWait();
  ...
  DLOG_TRACE << "The remote address " << remote_addr_
             << " is a host, try to resolve its IP address.";
  status_ = kDNSResolving;
  auto f = std::bind(&Connector::OnDNSResolved, shared_from_this(),
                     std::placeholders::_1);
  dns_resolver_ =
      std::make_shared<DNSResolver>(loop_, remote_host_, timeout_, f);
  dns_resolver_->Start();
}

void Connector::Cancel() {
  DLOG_TRACE << "Cancel to connect " << remote_addr_
             << " status=" << StatusToString();
  assert(loop_->IsInLoopThread());
  if (dns_resolver_) {
...
void Connector::Connect() {
  ...
  if (!laddr.empty()) {
    ...
    if (rc != 0) {
      int serrno = errno;
      EVLOG_ERROR << "bind failed, errno=" << serrno << " " << strerror(serrno);
      HandleError();
      return;
    }
  }
...
void Connector::HandleWrite() {
  ...
  if (status_ == kDisconnected) {
    ...
    // dispatched in the EventLoop pending task queue, and next loop time the
    // handle is invoked. So we need to check the status whether it is at a
    // kDisconnected
    EVLOG_INFO << "fd=" << chan_->fd() << " remote_addr=" << remote_addr_
  ...
  if (getsockopt(chan_->fd(), SOL_SOCKET, SO_ERROR, (char *)&err,
                 (socklen_t *)&len) != 0) {
    err = errno;
    EVLOG_ERROR << "getsockopt failed err=" << err << " " << strerror(err);
  }
...
void Connector::HandleError() {
  ...
  // to notify the user application layer in which the user maybe call
  // TCPClient::Disconnect. TCPClient::Disconnect may cause this Connector
  // object desctruct.
  auto self = shared_from_this();

  EVLOG_ERROR << "this=" << this << " status=" << StatusToString()
              << " fd=" << fd_ << " use_count=" << self.use_count()
              << " errno=" << serrno << " " << strerror(serrno);
  ...
  if (EVUTIL_ERR_CONNECT_REFUSED(serrno) ||
      !owner_tcp_client_->auto_reconnect()) {
    conn_fn_(-1, "");
  }

  // Although TCPClient has a Reconnect() method to deal with automatically
  // reconnection problem, TCPClient's Reconnect() will be invoked when a
  // established connection is broken down.
  //
  ...
  if (owner_tcp_client_->auto_reconnect()) {
    ...
    DLOG_TRACE << "loop=" << loop_ << " auto reconnect in "
               << owner_tcp_client_->reconnect_interval().Seconds()
               << "s thread=" << std::this_thread::get_id();
    loop_->RunAfter(owner_tcp_client_->reconnect_interval(),
                    std::bind(&Connector::Start, shared_from_this()));
  }
}

void Connector::OnConnectTimeout() {
  EVLOG_WARN << "this=" << this
             << " Connector::OnConnectTimeout status=" << StatusToString()
             << " fd=" << fd_ << " this=" << this;
...
void Connector::OnDNSResolved(const std::vector<struct in_addr> &addrs) {
  ...
  if (addrs.empty()) {
    EVLOG_ERROR << "this=" << this
                << " DNS Resolve failed. host=" << dns_resolver_->host();
    HandleError();
    return;
  }
