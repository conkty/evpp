...
bool EventLoopThreadPool::Start(bool wait_thread_started) {
  ...
  DLOG_TRACE << "thread_num=" << thread_num() << " base loop=" << base_loop_
             << " wait_thread_started=" << wait_thread_started;
  ...
  std::shared_ptr<std::atomic<uint32_t>> started_count(
      new std::atomic<uint32_t>(0));
  std::shared_ptr<std::atomic<uint32_t>> exited_count(
      new std::atomic<uint32_t>(0));
  for (uint32_t i = 0; i < thread_num_; ++i) {
    auto prefn = [this, started_count]() {
      DLOG_TRACE << "a working thread started tid="
    ...
    auto postfn = [this, exited_count]() {
      DLOG_TRACE << "a working thread exiting, tid="
                 << std::this_thread::get_id();
    ...
    if (!t->Start(wait_thread_started, prefn, postfn)) {
      // FIXME error process
      EVLOG_ERROR << "start thread failed!";
      return false;
    }
...
uint32_t EventLoopThreadPool::thread_num() const { return thread_num_; }

void EventLoopThreadPool::OnThreadStarted(uint32_t count) {
  DLOG_TRACE << "tid=" << std::this_thread::get_id() << " count=" << count
             << " started.";
...
void EventLoopThreadPool::OnThreadExited(uint32_t count) {
  ...
  if (count == thread_num_) {
    status_.store(kStopped);
    DLOG_TRACE
        << "this is the last thread stopped. Thread pool totally exited.";
    if (stopped_cb_) {
      stopped_cb_();
