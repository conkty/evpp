...
failed:
    ...
    if (retried_ < retry_number_) {
        EVLOG_WARN << "this=" << this << " http request failed : " << errmsg << " retried=" << retried_ << " max retry_time=" << retry_number_ << ". Try again.";
        Retry();
        return;
    }
...
void Request::HandleResponse(struct evhttp_request* r) {
    ...
    if (r) {
        ...
        if (!needs_retry || retried_ >= retry_number_) {
            EVLOG_WARN << "this=" << this << " response_code=" << r->response_code << " retried=" << retried_ << " max retry_time=" << retry_number_;
    ...
    // Retry
    if (retried_ < retry_number_) {
        EVLOG_WARN << "this=" << this << " response_code=" << (r ? r->response_code : 0) << " retried=" << retried_ << " max retry_time=" << retry_number_ << ". Try again";
        Retry();
        return;
    }
    ...
    if (!r) {
        ...
        while ((oslerr = bufferevent_get_openssl_error(conn_->bufferevent()))) {
            ...
            EVLOG_ERROR << "Openssl error: " << buffer;
            printed_some_error = true;
        }
        if (!printed_some_error) {
            EVLOG_ERROR << "socket error(" << errcode << "): "
