...
Conn::Conn(ConnPool *p, EventLoop *l)
    : loop_(l), pool_(p), host_(p->host()), port_(p->port())
#if defined(EVPP_HTTP_CLIENT_SUPPORTS_SSL)
      ,
      enable_ssl_(p->enable_ssl()), ssl_(nullptr), bufferevent_(nullptr)
#endif
      ,
      timeout_(p->timeout()), evhttp_conn_(nullptr) {
}
...
#endif
           Duration t)
    : loop_(l), pool_(nullptr), host_(h), port_(p)
#if defined(EVPP_HTTP_CLIENT_SUPPORTS_SSL)
      ,
      enable_ssl_(enable_ssl), ssl_(nullptr), bufferevent_(nullptr)
#endif
      ,
      timeout_(t), evhttp_conn_(nullptr) {
}
...
bool Conn::Init() {
  ...
  if (enable_ssl()) {
    ...
    bufferevent_ = bufferevent_openssl_socket_new(
        loop_->event_base(), -1, ssl_, BUFFEREVENT_SSL_CONNECTING,
        BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS);
  } else {
    bufferevent_ =
        bufferevent_socket_new(loop_->event_base(), -1, BEV_OPT_CLOSE_ON_FREE);
  }
  if (!bufferevent_) {
    EVLOG_ERROR << "bufferevent creation failed.";
    return false;
  }
...
#else
  evhttp_conn_ = evhttp_connection_base_new(loop_->event_base(), nullptr,
                                            host_.c_str(), port_);
#endif
  if (!evhttp_conn_) {
    EVLOG_ERROR << "evhttp_connection_new failed.";
    return false;
  }
