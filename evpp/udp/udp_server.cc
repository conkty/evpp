...
bool Server::Init(const std::string& listen_ports/*like "53,5353,1053"*/) {
    ...
    for (auto& s : vec) {
        ...
        if (i <= 0) {
            EVLOG_ERROR << "Cannot convert [" << s << "] to a integer. 'listen_ports' format wrong.";
            return false;
        }
        v.push_back(i);
    }
...
bool Server::Start() {
    if (!message_handler_) {
        EVLOG_ERROR << "MessageHandler DO NOT set!";
        return false;
    }
...
void Server::RecvingLoop(RecvThread* thread) {
    EVLOG_INFO << "UDPServer is running at 0.0.0.0:" << thread->port();
    ...
    while (true) {
        ...
        if (readn >= 0) {
            EVLOG_TRACE << "fd=" << thread->fd() << " port=" << thread->port()
            ...
            EVLOG_ERROR << "errno=" << eno << " " << strerror(eno);
        }
    }

    EVLOG_INFO << "fd=" << thread->fd() << " port=" << thread->port() << " UDP server existed.";
