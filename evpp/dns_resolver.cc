...
namespace evpp {
DNSResolver::DNSResolver(EventLoop *evloop, const std::string &h,
                         Duration timeout, const Functor &f)
    : loop_(evloop), dnsbase_(nullptr), dns_req_(nullptr), host_(h),
      timeout_(timeout), functor_(f) {
...
void DNSResolver::SyncDNSResolve() {
  ...
  if (err != 0) {
    EVLOG_ERROR << "this=" << this << " getaddrinfo failed. err=" << err << " "
                << gai_strerror(err);
  } else {
    for (struct addrinfo *rp = answer; rp != nullptr; rp = rp->ai_next) {
      ...
      DLOG_TRACE << "host=" << host_
                 << " resolved a ip=" << inet_ntoa(a->sin_addr);
    }
  }
...
void DNSResolver::AsyncWait() {
  ...
  timer_.reset(new TimerEventWatcher(
      loop_, std::bind(&DNSResolver::OnTimeout, this), timeout_));
...
void DNSResolver::AsyncDNSResolve() {
  ...
  assert(dnsbase_);
  dns_req_ = evdns_getaddrinfo(dnsbase_, host_.c_str(),
...
void DNSResolver::OnResolved(int errcode, struct addrinfo *addr) {
  if (errcode != 0) {
    if (errcode != EVUTIL_EAI_CANCEL) {
      ClearTimer();
      EVLOG_ERROR << "DNS resolve failed, "
                  << ", error code: " << errcode
    ...
    DLOG_WARN << "delete DNS base. errcode=" << errcode << " "
              << strerror(errcode);
  ...
  if (addr == nullptr) {
    EVLOG_ERROR << "this=" << this
                << " dns resolve error, addr can not be nullptr";
...
void DNSResolver::OnResolved(int errcode, struct addrinfo *addr, void *arg) {
  std::shared_ptr<DNSResolver> *pp =
      reinterpret_cast<std::shared_ptr<DNSResolver> *>(arg);
  EVLOG_TRACE << "this->use_count=" << pp->use_count();
...
void DNSResolver::OnResolved() {
  if (functor_) {
    ...
    // Sometimes, when it is timeout, this callback will be invoked in
    // OnTimeout() and `evdns_getaddrinfo_cancel(dns_req_)` will also invoke
