...
bool EventWatcher::Watch(Duration timeout) {
  ...
  if (attached_) {
    ...
    if (EventDel(event_) != 0) {
      EVLOG_ERROR << "event_del failed. fd=" << this->event_->ev_fd
                  << " event_=" << event_;
  ...
  if (EventAdd(event_, timeoutval) != 0) {
    EVLOG_ERROR << "event_add failed. fd=" << this->event_->ev_fd
                << " event_=" << event_;
    return false;
  }
  attached_ = true;
...
PipeEventWatcher::~PipeEventWatcher() { Close(); }
...
bool PipeEventWatcher::DoInit() {
  ...
  if (evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, pipe_) < 0) {
    int err = errno;
    EVLOG_ERROR << "create socketpair ERROR errno=" << err << " "
                << strerror(err);
    goto failed;
  }
...
void PipeEventWatcher::HandlerFn(evpp_socket_t fd, short /*which*/, void *v) {
  EVLOG_INFO << "PipeEventWatcher::HandlerFn fd=" << fd << " v=" << v;
...
bool PipeEventWatcher::AsyncWait() { return Watch(Duration()); }
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
TimerEventWatcher::TimerEventWatcher(EventLoop *loop, const Handler &handler,
                                     Duration timeout)
    : EventWatcher(loop->event_base(), handler), timeout_(timeout) {}

TimerEventWatcher::TimerEventWatcher(EventLoop *loop, Handler &&h,
                                     Duration timeout)
...
TimerEventWatcher::TimerEventWatcher(struct event_base *loop, Handler &&h,
                                     Duration timeout)
...
void TimerEventWatcher::HandlerFn(evpp_socket_t /*fd*/, short /*which*/,
                                  void *v) {
  TimerEventWatcher *h = (TimerEventWatcher *)v;
  h->handler_();
}

bool TimerEventWatcher::AsyncWait() { return Watch(timeout_); }
...
void SignalEventWatcher::HandlerFn(signal_number_t /*sn*/, short /*which*/,
                                   void *v) {
  SignalEventWatcher *h = (SignalEventWatcher *)v;
  h->handler_();
}

bool SignalEventWatcher::AsyncWait() { return Watch(Duration()); }
} // namespace evpp
