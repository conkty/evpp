...
bool EventLoopThread::Start(bool wait_thread_started, Functor pre,
                            Functor post) {
  DLOG_TRACE;
  status_ = kStarting;
  ...
  thread_.reset(
      new std::thread(std::bind(&EventLoopThread::Run, this, pre, post)));
...
void EventLoopThread::Stop(bool wait_thread_exit) {
  DLOG_TRACE << "loop=" << event_loop_
             << " wait_thread_exit=" << wait_thread_exit;
...
void EventLoopThread::Join() {
  ...
  if (thread_ && thread_->joinable()) {
    DLOG_TRACE << "loop=" << event_loop_ << " thread=" << thread_
               << " joinable";
    try {
      thread_->join();
    } catch (const std::system_error &e) {
      EVLOG_ERROR << "Caught a system_error:" << e.what()
                  << " code=" << e.code();
    }
    thread_.reset();
  }
...
void EventLoopThread::set_name(const std::string &n) { name_ = n; }

const std::string &EventLoopThread::name() const { return name_; }

EventLoop *EventLoopThread::loop() const { return event_loop_.get(); }
...
void EventLoopThread::AfterFork() { loop()->AfterFork(); }
