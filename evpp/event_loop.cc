...
EventLoop::EventLoop(struct event_base *base)
    : evbase_(base), create_evbase_myself_(false), notified_(false),
  ...
  assert(rc);
  if (!rc) {
    EVLOG_FATAL << "PipeEventWatcher init failed.";
  }
...
void EventLoop::Run() {
  ...
  if (rc == 1) {
    EVLOG_ERROR << "event_base_dispatch error: no event registered";
  } else if (rc == -1) {
    int serrno = errno;
    EVLOG_ERROR << "event_base_dispatch error " << serrno << " "
                << strerror(serrno);
  }
...
void EventLoop::AfterFork() {
  ...
  if (rc != 0) {
    EVLOG_FATAL << "event_reinit failed!";
    abort();
  }
...
void EventLoop::QueueInLoop(const Functor &cb) {
  ...
  if (!notified_.load()) {
    ...
    //  2. On thread2 we watched this event, so wakeup the CPU changed to run
    //  this EventLoop on thread2 and executed all the pending task
    //  3. Then the CPU changed to run on thread1 and set notified_ to true
    //  4. Then, some thread except thread2 call this QueueInLoop to push a task
    //  into the queue, and find notified_ is true, so there is no change to
    //  wakeup thread2 to execute this task
...
void EventLoop::QueueInLoop(Functor &&cb) {
  ...
  if (!notified_.load()) {
    ...
    if (watcher_) {
      ...
      DLOG_TRACE << "watcher_ is empty, maybe we call EventLoop::QueueInLoop "
                    "on a stopped EventLoop. status="
                 << StatusToString();
      assert(!IsRunning());
    }
  } else {
